...
and macOS. `free` uses `totalSystemMemory` for RAM, then
real page counts: FreeBSD `sysctlbyname`, OpenBSD
`CTL_VM`/`VM_UVMEXP`, NetBSD `CTL_VM`/`VM_UVMEXP2`. `ln`
hard-link tests join `TestDir.getBasePath()` instead of
