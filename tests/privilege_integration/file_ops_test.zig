//! Integration tests for file operation workflows under privilege
//! simulation. These tests verify complex file operations across multiple
//! utilities.
//!
//! Prerequisites:
...
test "privileged: echo and cat with permission restrictions" {
    ...
    // Try to write to the read-only file. Under fakeroot, permission
    // checks might be bypassed for some operations, so we just test the
...
test "privileged: pwd with restricted directory access" {
    ...
    try testing.expect(std.mem.endsWith(u8, std.mem.trimEnd(u8, pwd_result.stdout, "\n"), "/workspace/project/src"));
...
test "privileged: complex permission inheritance" {
    ...
    try testing.expect(std.mem.find(u8, ls_result.stdout, "child") != null);
    try testing.expect(std.mem.find(u8, ls_result.stdout, "grandchild") != null);
