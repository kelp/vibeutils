...
test_mv() {
    ...
    #   $ LC_ALL=C /usr/bin/mv --v
    #   /usr/bin/mv: option '--v' is ambiguous; possibilities: '--verbose' '--version'
    ...
    local abbrev_v_expected="mv: option '--v' is ambiguous; possibilities: '--verbose' '--version'"$'\n'"Try 'mv --help' for more information."
    if [[ $abbrev_v_exit -eq 1 && "$abbrev_v_err" == "$abbrev_v_expected" ]]; then
        print_test_result "mv --v lists candidates in GNU longopts order" "PASS"
    else
        print_test_result "mv --v lists candidates in GNU longopts order" "FAIL" \
