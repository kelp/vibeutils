...
test_rmdir() {
    ...
    #   $ LC_ALL=C /usr/bin/rmdir --v
    #   /usr/bin/rmdir: option '--v' is ambiguous; possibilities: '--verbose' '--version'
    ...
    local abbrev_v_expected="rmdir: option '--v' is ambiguous; possibilities: '--verbose' '--version'"$'\n'"Try 'rmdir --help' for more information."
    if [[ $abbrev_v_exit -eq 1 && "$abbrev_v_err" == "$abbrev_v_expected" ]]; then
        print_test_result "rmdir --v lists candidates in GNU longopts order" "PASS"
    else
        print_test_result "rmdir --v lists candidates in GNU longopts order" "FAIL" \
