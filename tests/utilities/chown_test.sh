...
test_chown() {
    ...
    #   $ LC_ALL=C /usr/bin/chown --v
    #   /usr/bin/chown: option '--v' is ambiguous; possibilities: '--verbose' '--version'
    ...
    local abbrev_v_expected="chown: option '--v' is ambiguous; possibilities: '--verbose' '--version'"$'\n'"Try 'chown --help' for more information."
    if [[ $abbrev_v_exit -eq 1 && "$abbrev_v_err" == "$abbrev_v_expected" ]]; then
        print_test_result "chown --v lists candidates in GNU longopts order" "PASS"
    else
        print_test_result "chown --v lists candidates in GNU longopts order" "FAIL" \
