...
test_ln() {
    ...
    #   $ LC_ALL=C /usr/bin/ln --v
    #   /usr/bin/ln: option '--v' is ambiguous; possibilities: '--verbose' '--version'
    ...
    local abbrev_v_expected="ln: option '--v' is ambiguous; possibilities: '--verbose' '--version'"$'\n'"Try 'ln --help' for more information."
    if [[ $abbrev_v_exit -eq 1 && "$abbrev_v_err" == "$abbrev_v_expected" ]]; then
        print_test_result "ln --v lists candidates in GNU longopts order" "PASS"
    else
        print_test_result "ln --v lists candidates in GNU longopts order" "FAIL" \
