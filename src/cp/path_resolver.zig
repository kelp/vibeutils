...
pub const PathResolver = struct {
    ...
    pub fn resolveFinalDestination(allocator: std.mem.Allocator, source: []const u8, dest: []const u8) ![]u8 {
...
test "PathResolver: resolveFinalDestination" {
    ...
    const resolved_to_dir = try PathResolver.resolveFinalDestination(testing.allocator, source_path, dir_path);
    ...
    const resolved_to_file = try PathResolver.resolveFinalDestination(testing.allocator, source_path, nonexistent_path);
