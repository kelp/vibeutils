...
pub const CopyEngine = struct {
    ...
    pub fn executeCopy(self: *CopyEngine, stderr_writer: anytype, operation: types.CopyOperation) !void {
        ...
        if (self.ctx.options.interactive and operation.dest_exists) {
            const should_proceed = try user_interaction.UserInteraction.shouldOverwrite(stderr_writer, operation.final_dest_path);
            if (!should_proceed) {
        ...
        if (operation.dest_exists) {
            ...
            if (is_same) {
                ...
                errors.ErrorHandler.reportError(stderr_writer, context, errors.CopyError.SameFile);
                self.stats.addError();
        ...
        switch (operation.source_type) {
            .regular_file => {
                try file_handlers.FileHandlers.copyRegularFile(stderr_writer, self.ctx, operation, &self.stats);
            },
            .symlink => {
                if (self.ctx.options.no_dereference) {
                    try file_handlers.FileHandlers.copySymlink(stderr_writer, self.ctx, operation, &self.stats);
                } else {
            ...
            .directory => {
                try file_handlers.FileHandlers.copyDirectory(stderr_writer, self.ctx, operation, &self.stats);
            },
            .special => {
                const context = errors.ErrorContext{
    ...
    pub fn executeCopyBatch(self: *CopyEngine, stderr_writer: anytype, operations: []types.CopyOperation) !void {
        for (operations, 0..) |operation, i| {
            // Show progress for large batches
            if (operations.len > 5) {
                try user_interaction.UserInteraction.showProgress(stderr_writer, i + 1, operations.len, operation.source);
            }
        ...
        if (operations.len > 5) {
            try user_interaction.UserInteraction.clearProgress(stderr_writer);
        }
    }

    /// Validate that an operation is safe to execute
    pub fn validateOperation(self: *CopyEngine, stderr_writer: anytype, operation: types.CopyOperation) !void {
        // Validate paths
        ...
        if (!source_exists) {
            ...
            errors.ErrorHandler.reportError(stderr_writer, context, errors.CopyError.SourceNotFound);
        ...
        if (operation.source_type == .directory and !self.ctx.options.recursive) {
            ...
            errors.ErrorHandler.reportError(stderr_writer, context, errors.CopyError.RecursionNotAllowed);
        ...
        if (operation.dest_exists and !self.ctx.options.force and !self.ctx.options.interactive) {
            ...
            errors.ErrorHandler.reportError(stderr_writer, context, errors.CopyError.DestinationExists);
...
test "CopyEngine: validate operation errors" {
    ...
    try testing.expectError(errors.CopyError.SourceNotFound, engine.validateOperation(stderr_writer, bad_operation));
}
