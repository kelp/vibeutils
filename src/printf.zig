...
const enable_fuzz_tests = common.fuzz.shouldFuzzUtility("printf");

test "printf fuzz intelligent" {
    if (!enable_fuzz_tests) return error.SkipZigTest;
    try std.testing.fuzz(testing.allocator, testPrintfIntelligentWrapper, .{});
}

fn testPrintfIntelligentWrapper(allocator: std.mem.Allocator, input: []const u8) !void {
    if (!common.fuzz.shouldFuzzUtilityRuntime("printf")) return;
    ...
    _ = runPrintf(allocator, args, stdout_buf.writer(allocator), common.null_writer) catch {
        return;
    };
}

test "printf fuzz format strings" {
    if (!enable_fuzz_tests) return error.SkipZigTest;
    try std.testing.fuzz(testing.allocator, testPrintfFormatStrings, .{});
}

fn testPrintfFormatStrings(allocator: std.mem.Allocator, input: []const u8) !void {
    if (!common.fuzz.shouldFuzzUtilityRuntime("printf")) return;
    if (input.len == 0) return;

    // Use the raw input as a format string
    const args = [_][]const u8{ input, "42", "hello", "3.14" };
...
test "printf fuzz escape sequences" {
    if (!enable_fuzz_tests) return error.SkipZigTest;
    try std.testing.fuzz(testing.allocator, testPrintfEscapeSequences, .{});
}

fn testPrintfEscapeSequences(allocator: std.mem.Allocator, input: []const u8) !void {
    if (!common.fuzz.shouldFuzzUtilityRuntime("printf")) return;
    ...
    const args = [_][]const u8{escape_seq};
    var stdout_buf = try std.ArrayList(u8).initCapacity(allocator, 0);
    defer stdout_buf.deinit(allocator);
