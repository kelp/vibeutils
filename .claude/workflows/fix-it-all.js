...
// Capped at three units. Width barely moves total wall clock — the harness
// allows min(16, cores-2) concurrent agents per workflow invocation, so a wider
// wave queues more work behind the same slots rather than running faster. What
// width actually costs is disk: every unit gets its own worktree carrying
// 1.3-3.6 GB of .zig-cache (more once green starts running full suites), and
// OrbStack charges those bytes to the host and the VM at once.
//
// The cap exists because the original theme-based grouping put 7 modules in one
// wave, which would have meant ~21 GB of simultaneous cache and 7 concurrent
// full-suite runs in the green phase.
const SHARED_WAVES = [
  ...
  { id: 'S3', why: 'permissions, identity, and paths', modules: ['mode', 'user_group', 'path'] },
  { id: 'S4', why: 'globbing, environment, shared constants', modules: ['glob', 'env', 'constants'] },
  { id: 'S5', why: 'user-facing output plumbing', modules: ['help', 'prompt', 'format'] },
  { id: 'S6', why: 'entry points and the library root', modules: ['main', 'lib'] },
  { id: 'S7', why: 'glyphs and width computation', modules: ['icons', 'unicode'] },
  { id: 'S8', why: 'display configuration and color', modules: ['display_config', 'style', 'colors'] },
  { id: 'S9', why: 'terminal capability and time formatting', modules: ['terminal', 'time', 'relative_date'] },
  { id: 'S10', why: 'repository state and the dormant-test lint', modules: ['git', 'force_import_lint'] },
  { id: 'S11', why: 'test infrastructure; this repo has shipped 272 dormant tests', modules: ['test_utils', 'test_utils_privilege', 'test_dir'] },
  { id: 'S12', why: 'the privilege harnesses', modules: ['privilege_test', 'privilege_test_integration'] },
];
...
// Same cap. The eight utilities over 4000 lines run alone: their green phase is
// where the build cache balloons, and a wave is only as done as its slowest
// unit. Everything else groups by three. Ordering is still by
// implementation-size-to-test-coverage gap.
const UTIL_WAVES = [
  { id: 'U0', utils: ['whoami', 'true', 'false'] },
  { id: 'U1', utils: ['df'] },
  { id: 'U2', utils: ['du'] },
  { id: 'U3', utils: ['dd'] },
  { id: 'U4', utils: ['free', 'sort', 'seq'] },
  { id: 'U5', utils: ['id', 'nl', 'tr'] },
  { id: 'U6', utils: ['cut', 'date', 'timeout'] },
  { id: 'U7', utils: ['uniq', 'tac', 'env'] },
  { id: 'U8', utils: ['realpath', 'readlink', 'mktemp'] },
  { id: 'U9', utils: ['find'] },
  { id: 'U10', utils: ['stat'] },
  { id: 'U11', utils: ['printf', 'test', 'mv'] },
  { id: 'U12', utils: ['ls'] },
  { id: 'U13', utils: ['cp'] },
  { id: 'U14', utils: ['grep'] },
  { id: 'U15', utils: ['chmod', 'chown', 'rm'] },
  { id: 'U16', utils: ['rmdir', 'mkdir', 'ln'] },
  { id: 'U17', utils: ['touch', 'tail', 'head'] },
  { id: 'U18', utils: ['wc', 'cat', 'tee'] },
  { id: 'U19', utils: ['sleep', 'echo', 'yes'] },
  { id: 'U20', utils: ['basename', 'dirname', 'pwd'] },
];
...
function wtPreamble(c) {
  const lines = [
    ...
    '',
    'BLAST-RADIUS RULES — these are absolute, and they outrank any instruction below telling you to',
    'reproduce, verify, or investigate something. This is a real person\'s laptop, not a sandbox.',
    '',
    '1. NEVER run a destructive command against a real system path. Not `/`, not `$HOME`, not `/usr`,',
    '   `/etc`, `/Users`, `/Library`, `/var`, and not any path you did not create yourself. This',
    '   includes chmod, chown, rm, mv, truncate, dd, and the utilities under audit, which ARE these',
    '   commands. A previous run invoked our own chmod as',
    '     ./zig-out/bin/chmod -R --no-preserve-root 755 /',
    '   on the host to test the preserve-root failsafe. It walked the real filesystem and was stopped',
    '   only by a timeout. Do not do this or anything like it.',
    '',
    '2. To exercise destructive or system-scope behavior, BUILD A FIXTURE and point the command at the',
    '   fixture: `mkdir -p /tmp/<unique>/root/{a,b}` then operate on `/tmp/<unique>/root`. A failsafe',
    '   like preserve-root is verified by observing the REFUSAL (run it and read the error and exit',
    '   code), never by overriding the failsafe and letting the operation proceed.',
    '',
    '3. If a check genuinely requires operating on `/` or on system paths, run it INSIDE the Linux VM',
    `   (\`${COMMON.linux_prefix} ...\`), which is disposable, and never on the host. When in doubt,`,
    '   the VM is the answer.',
    '',
    '4. NEVER delete anything you did not create. Not caches, not build output outside your worktree,',
    '   not temp files belonging to other processes, and never by wildcard or glob across a shared',
    '   directory. `~/Library/Caches`, `~/.cache`, and /tmp are shared with the user and other agents.',
    '   If disk space seems short, say so in your result; do not free it yourself.',
    '',
    '5. Anything you mutate for an experiment — a sabotage edit, a fixture, an env change — you revert',
    '   before finishing, and you say in your result that you reverted it.',
    '',
    'If following these rules means you cannot fully verify something, that is the correct outcome:',
    'report what you could establish and what you could not, and why. An unverified finding is cheap.',
    'An unrecoverable laptop is not.',
    '',
    `Unit: ${c.util} (${c.kind})`,
    `Source: ${c.src.join(', ')}`,
